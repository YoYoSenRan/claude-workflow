...
├── agents/<name>/              (后续 agent 加测在这)
├── mcps/<name>/                (后续 MCP server 加测在这)
└── tools/<name>/               (后续 CLI 加测在这)
```

**为什么不放 skills/<name>/tests/?** — 测试不是 skill 的功能内容, 跟 SKILL.md 同目录混在一起污染 skill 包。顶级 tests/ 镜像根结构, 视觉清爽, 跑测路径统一。
