...
- [ ] 步 5：每个任务含 Files + Steps（checkbox）+ 完整代码块 + Run 命令 + Expected 输出
...
## 反模式（不应出现）
- ✗ 直接给「几步流程」，不写文件
- ✗ 任务含 `TODO` / `TBD` / `// 类似 Task N`
- ✗ 任务 step 不给完整代码（只写「实现登录」）
- ✗ 不给 Run 命令 + Expected
- ✗ 任务粒度 >5 分钟（整个 Task 1 干完一个大模块）
...
- TDD 豁免：「写个 README 介绍这个模块」 → 触发但任务不加 test step
