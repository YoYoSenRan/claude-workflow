---
name: worktree
description: 创建隔离工作区，避免进行中的改动影响主分支
---

# 使用 Git 工作树

确保工作在一个隔离的目录里进行。优先用当前环境自带的工作树工具。只有在没有自带工具时，才手动用 git 工作树。先看看是不是已经在隔离环境里了，然后试自带工具，最后才用 git。不要跟当前环境的机制对着干。
...
**如果 `GIT_DIR != GIT_COMMON`（且不在子模块中）：** 你已经在一个隔离的工作目录里了。跳到步骤 3（项目初始化）。不要再创建另一个工作树。
...
自带工具会自动处理目录位置、分支创建和清理。如果有自带工具你还手动跑 `git worktree add`，会产生当前环境看不见也管不了的残留。
...
**权限不够时的处理：** 如果 `git worktree add` 因权限错误失败，告诉用户权限不允许创建工作树，你将改为在当前目录工作。然后在原地执行 setup 与基线测试。
...
| 错误 | 问题 | 正确做法 |
|------|------|----------|
| 跟当前环境对着干 | 环境已经提供隔离了还手动 `git worktree add` | 步骤 0 先检测，步骤 1a 优先用自带工具 |
...
| 在测试失败时继续 | 分不清是你弄坏的还是本来就坏的 | 汇报失败，得到明确同意再继续 |

## 快速参考

| 情况 | 行动 |
|-----------|--------|
| 已经在隔离的工作目录里 | 跳过创建（步骤 0） |
| 在子模块中 | 视为普通仓库（步骤 0 的防护） |
| 有自带的工作树工具 | 用它（步骤 1a） |
| 没有自带工具 | 手动用 git 工作树（步骤 1b） |
