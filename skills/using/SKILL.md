...
收到用户请求后，必须先做最小路由判断：选择能安全完成任务的最小流程——小任务不流程化，模糊任务不盲改，高风险任务不跳过计划和验证。
某个 skill 明确适用时，必须调用并遵守；不要因为“可能相关”就把低风险小改升级成完整流程。
</EXTREMELY-IMPORTANT>
...
用户定义做什么，skill 定义怎么做。冲突时遵循用户指令，并简短说明被跳过的流程风险。

## 基本流程

1. 判断请求类型、明确程度和风险，选最小足够流程。
2. 涉及代码实现、计划、调试、测试时，先检查项目级 rules / skills / references。
3. 某 skill 明确适用，先用 `Skill` 工具加载并按原文执行（不凭记忆，不用普通文件读取代替）。
4. 调用主流程 skill 时公开说明：`正在使用 <skill> 来 <目的>`。
5. 轻量小改只需简短说明在做什么；非轻量任务有清单就用 TodoWrite。
...
项目存在 `CLAUDE.md`、`.claude/CLAUDE.md` 或 `.claude/rules/` 时，它们是项目级持续规则，优先于通用 workflow 默认建议。

项目存在 setup 生成的 `.claude/skills/` 时：先按实际存在的 `.claude/skills/*/SKILL.md` 名称匹配当前任务；只加载最相关的一个；找不到匹配就改读适用的 `.claude/rules/`（reference 由对应 skill 自己 Read）；项目画像、命令矩阵、扫描报告优先按 reference 处理；skill 按项目真实任务能力命名。
