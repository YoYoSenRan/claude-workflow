...
- 简单但仍需对齐的改动：在对话里给出短设计、方案取舍和成功标准即可，不必写成文件。
...
**进入 plan 前的判断：**
设计被确认后，先说明你对改动复杂度的判断，再进入 plan。不要用文件数量判断复杂度；一个文件也可能包含复杂状态、资源生命周期、用户可见行为或难验证的逻辑。
...
全部满足时，告诉用户这是低风险改动，进入 plan 后给轻量内联计划。只要有一项不满足，告诉用户需要更完整的计划，进入 plan 后写 `docs/plans/...`。
