---
name: think
description: "在任何创造性工作之前你必须使用本技能——理解需求、消除歧义、对齐意图之前，禁止动手。它在执行之前先把模糊 prompt 打磨成具体可执行的任务。"
when_to_use: "用户原话像 '帮我看下'、'处理一下'、'改改'、'优化下'、'重构这块'、'看看这个项目'、'怎么搞这个 bug'、范围或目标不清晰的任何请求。"
---

# 把模糊 prompt 打磨成对齐意图

通过自然的协作对话，把"看下""改改""处理一下"这类多解读的请求，打磨成具体、可执行、用户确认过的任务描述。
...
每个模糊请求都要走这套流程。一行配置、一个小重构、一处文案改动——全都要走。"简单"的模糊请求恰恰是未经审视的假设造成最多无效工作的地方。对齐可以很短（真正简单的请求一句话复述就够），但你必须复述并获得确认。
...
7. **过渡** —— 用户拍板后，按任务类型调对应的下一个技能（见下文）

> **不要写任务描述文档。** 任务拆解、文件路径、验证命令、TDD 步骤是 `plan` 技能的活。think 的产出**只是口头对齐过的方向**，不落盘、不写五字段模板。要落文档就交给 `plan`。
...
**终止状态是过渡到下一个技能。** 不要在本技能内直接写代码、跑命令、改文件。对齐完成之后唯一可以做的事是调用下一个合适的技能（见下文过渡表）。

## 详细流程

**扫歧义信号：**

收到 prompt 后第一件事是静默扫描，任一命中即触发本技能。
...
每条消息只问一个话题——如果话题需要更多探索，就拆成多条问题，一次一个。
...
- 对每个单元应当能回答：要做什么、改哪些文件、怎么验证成功？
- 别人不读你的解释就能知道这个任务做什么吗？
- 如果不能，那么边界划得还不够清——继续追问
...
等用户回复。要求改动则回到"复述 → 提问"再跑一轮。用户拍板后才能过渡。

**不要写任务描述文档。** think 不产出 `docs/...md`，不写"任务/范围/目的/不做/验证"五字段模板。这些是 `plan` 技能的产物，think 写就是越界。

**过渡到下一个技能：**

按任务类型调对应技能，**不要在本技能内动手**：
...
| "我已经理解了" | 没复述出来就是没理解。 |
| "complexity 太高不可能拆" | 拆不动就是范围没界定，继续问。 |
