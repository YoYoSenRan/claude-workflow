...
读取时机：`setup` 已形成初步候选、需要判断是否派发深度扫描 agent 时读取；轻量项目且无深扫需求时不要读取。

产出：必须派发的 `subagent_type`、子代理提示、返回表格和主线程合并规则。
...
| 存在项目特有的样式 / 界面 / 展示层规范（如 CSS 体系、UI 主题、终端样式、报表模板等） | `setup-styling` |
| 存在项目核心框架、高复用抽象层或领域 DSL（如脚手架生成器、ORM 封装、配置驱动引擎、API factory 等） | `setup-framework` |
| 存在 `CLAUDE.md`、`.claude/CLAUDE.md` 或 `.claude/rules/*.md` | `setup-rules` |
| 存在反复出现的文件骨架、业务流程或目录组织模式 | `setup-patterns` |
| 存在高频且有项目特有流程 / 风险 / 规则的业务领域 | 按领域派发 `setup-domain` |
| 用户明确要求 agent / 深度扫描 | 根据候选同时派发 `setup-config`、`setup-conventions`，存在展示层规范时加 `setup-styling`，存在核心抽象时加 `setup-framework`，必要时加 `setup-patterns` / `setup-rules` / `setup-domain` |
...
| `setup-conventions` | 项目中各类代表文件（按文件索引中的高频类型选取） | 严格代码规范：命名、声明顺序、导入导出顺序、注释、函数拆分、内聚耦合、默认质量基线 |
| `setup-styling` | 项目的展示层 / 样式规范相关文件（CSS 体系、UI 主题、终端输出格式、报表模板等） | 严格展示层规范：命名、嵌套层级、属性顺序、单位 / 变量复用、状态表达、覆盖边界和平台差异 |
| `setup-framework` | 项目核心框架和高复用抽象（ORM 封装、脚手架生成器、配置驱动引擎、API factory、核心中间件等） | 核心抽象心智模型、定义入口、使用入口、配置协议、方法能力、扩展步骤、验证方式和禁忌 |
| `setup-patterns` | 业务模块中的重复骨架和协作文件 | 文件骨架、目录组织、业务流程、代表样例；不能替代 `setup-conventions` / `setup-styling` / `setup-framework` |
...
| 发现 | 强度 | 影响范围 | 任务触发 | 编排消费 | 产物建议 | 生成理由 | 证据文件 |
|---|---|---|---|---|---|---|---|
...
- 产物建议只能是：rule / skill / reference / internal。
- 任务触发必须写明什么用户任务会用到它。
- 编排消费必须写明它会被 `think` 的哪个 workflow 阶段消费，例如：任务判断 / 项目增强读取 / 计划前置 / 验证选择 / review 检查。
...
| 长清单、配置矩阵、模块 API、示例、证据、扫描报告 | reference |
...
候选进入 rules 或 skills 前，必须写清它会如何改变模型下一步行为，以及 `think` 会在 workflow 编排中的哪个阶段加载它；只能描述“项目里有什么”的候选放入 reference 或 internal。
