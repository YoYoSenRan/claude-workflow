...
完整 setup 中，setup report、domains 和 habits 必须作为 references 候选；放在哪个 skill 的 `references/` 下，取决于最终是否存在对应任务 skill。没有对应 skill 时，可以建议放到 `.claude/references/` 或项目入口引用的文档位置。
...
- 需要同时分析配置命令、代码规范、样式规范、核心框架、编码习惯、多个业务领域；
...
| 路径 | 承载内容 | 用途 |
|---|---|---|
| `.claude/references/<file>.md` 或 `.claude/skills/<name>/references/<file>.md` | <样例、习惯矩阵、命令表、setup report 等长内容> | <哪个入口会读取它；解决什么查找或证据问题> |
```
