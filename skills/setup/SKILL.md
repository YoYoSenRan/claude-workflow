...
**核心原则：** 只把会改变未来行动的内容写入项目配置。普通项目画像、单点观察、无触发条件的长文，不生成。
...
生成 reference，当内容太长，不适合放在 rule 或 SKILL.md 正文里，并且有明确消费它的 skill。
...
| "把扫描报告放进 reference" | reference 必须被某个 skill 消费。 |
