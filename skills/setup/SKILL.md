...
生成 proposal 前必须先形成扫描账本。没有扫描账本，不允许说“扫描完成”，也不允许给最终候选。扫描账本用于内部判断和用户追问时核对；默认用户输出只呈现已分析范围和建议添加项，不展示“不适用 / 证据不足 / 未生成”清单。
</HARD-GATE>
...
默认由主线程完成深度扫描。遇到以下情况，可以并发派只读子代理：
...
派发前读取 `references/agents.md`，按其中推荐分工和提示模板组织任务。
...
- 可在领域 skill 中引用；
- 可避免重复生成同类 rule；
- 不能作为“不生成 context / commands / 高频领域 skill”的理由。
...
| 类型 | 名称 / 路径 | 作用 | 内容概要 | 依据 |
|---|---|---|---|---|
| rule | `.claude/rules/<name>.md` | <短规则解决什么问题> | <包含哪些规则> | <关键证据> |
| skill | `.claude/skills/<name>/SKILL.md` | <什么任务会加载> | <触发、执行顺序、验证方式> | <关键证据> |
| reference | `.claude/skills/<name>/references/<file>.md` | <承载什么长证据> | <样例、习惯矩阵、命令表> | <关键证据> |
```
...
- 有证据才生成，没有证据就不生成；默认不向用户列“不适用 / 证据不足 / 未生成”。
